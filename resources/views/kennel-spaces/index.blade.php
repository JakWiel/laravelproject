...
@section('content')
            ...
            <tbody>
                @forelse ($models as $model)
                    <tr class="align-middle text-center">
                        ...
                        <td>{{ $model->name }}</td>
                        <td>{{ $model->size }}</td>
                        <td>{{ $model->availability_status }}</td>
                        <td>{{ $model->isActive ? '✅' : '❌' }}</td>
                        <td>{{ $model->dateCreated }}</td>
                        <td>{{ $model->dateEdited ?? 'N/A' }}</td>
                        <td>{{ $model->dateDeleted ?? 'N/A' }}</td>
                        <td>
                            <a href="kennel-spaces/edit/{{$model->id}}" class="btn btn-sm btn-warning edit-event"
    ...
    <script>
        ...
        $(".edit-event").on("click", function (e) {
            ...
            $.ajax({
                url: "/kennel-spaces/edit/" + clickedId,
                method: "get",
                data: { _token: "{{ csrf_token() }}" },
                dataType: "html",
                success: function (result) {
